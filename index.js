	...
	async function main() {
		...
		class Toolbar extends ol.control.Control {
			...
			constructor() {
				...
				this.#expandButton.classList.add('fa-solid-angle-up');
  	...
  	`);
		...
		/* Удаление ненужного, переносы, переименования */
		{
			...
			layersButton.classList.add('fa-solid-layer-group');

			zoomContainer.append(rotateArrow, toggleFollow, layersButton);

			toggleFollow.innerText = '';
			toggleFollow.classList.add('fa-solid-location-crosshairs');
		...
		/* Меню настроек */
		{
			...
			settingsButton.classList.add('fa-solid-gears');
		...
		/* Кнопка обновления страницы */
		{
			if (window.navigator.userAgent.toLowerCase().includes('wv')) {
				...
				reloadButton.classList.add('fa-solid-rotate');
		...
		/* Избранные точки */
		{
			...
			/* Звезда на карточке точки */
			{
				...
				star.classList.add('sbgcui_button_reset', 'sbgcui_point_star', `fa-${favorites[guid]?.isActive ? 'solid' : 'regular'}-star`);
			...
			/* Список избранных */
			{
				...
				function fillFavsList() {
					...
					for (let guid in favorites) {
						if (favorites[guid].isActive) {
							...
							deleteButton.classList.add('sbgcui_button_reset', 'sbgcui_favs-li-delete', 'fa-solid-circle-xmark');
				...
				star.classList.add('fa-solid-star', 'sbgcui_button_reset');
		...
		/* Дискавер без рефа или предметов */
		{
			...
			noLootSpan.classList.add('sbgcui_no_loot', 'fa-solid-droplet-slash');
			noRefsSpan.classList.add('sbgcui_no_refs', 'fa-solid-link-slash');
		...
		/* Сортировка рефов */
		{
			...
			sortOrderButton.classList.add('fa-solid-sort', 'sbgcui_button_reset', 'sbgcui_refs-sort-button');
		...
		/* Перезапрос инвью */
		{
			...
			button.classList.add('fa-solid-rotate');
		...
		/* Выбор точки из кластера */
		{
			...
			closeButton.classList.add('sbgcui_button_reset', 'sbgcui_cluster-close', 'fa-solid-circle-xmark');
		...
		/* Режим рисования звезды */
		{
			...
			starModeButton.classList.add('fa-solid-asterisk');
		...
		/* Переключение между точками */
		{
			...
			arrow.classList.add('sbgcui_swipe-cards-arrow', 'fa-solid-angles-left');
		...
		/* Сравнение статы со своей */
		{
			...
			compareButton.classList.add('fa-solid-scale-unbalanced', 'sbgcui_profile-compare');
		...
		/* Кнопка смены сортировки при рисовании */
		{
			...
			flipButton.classList.add('fa-solid-arrow-down-short-wide', 'fa-rotate-270', 'sbgcui_drawslider_sort');
